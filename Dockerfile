...
# and `pip install youtube_analysis` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "youtube_analysis", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n youtube_analysis -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "youtube_analysis", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "youtube_analysis", "YOUR_CONSOLE_SCRIPT"]
